...
export async function GET(request: Request) {
  ...
  const cookieValue = jar.get(oauthCookieName("google"))?.value;
  try {
    ...
    const identity = await exchangeOAuthCode("google", code, saved.verifier);
    ...
    response.cookies.set(oauthCookieName("google"), "", { ...oauthCookieOptions(), maxAge: 0 });
    return response;
  } catch (error) {
