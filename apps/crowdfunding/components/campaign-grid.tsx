...
export function CampaignGrid() {
  return <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">{campaigns.map((campaign) => {
    ...
    return <Card key={campaign.slug} className="group overflow-hidden transition duration-200 hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(0,0,0,.08)]">
      ...
      <div className="p-5"><p className="line-clamp-3 min-h-[72px] text-sm leading-6 text-neutral-600">{campaign.summary}</p><div className="mt-5 flex items-end justify-between"><div><div className="text-lg font-bold">€{campaign.raised.toLocaleString()}</div><div className="mt-0.5 text-[10px] font-medium text-neutral-500">reconciled funding</div></div><div className="text-right"><div className="text-xs font-semibold text-neutral-600">{pct}%</div><div className="mt-0.5 text-[10px] text-neutral-500">of €{campaign.goal.toLocaleString()}</div></div></div><div className="mt-3"><ProgressBar value={pct}/></div><div className="mt-5 flex items-center justify-between border-t border-neutral-200 pt-4"><StatusBadge tone="success">Verified treasury</StatusBadge><a href={`/campaigns/${campaign.slug}`} className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-950 transition group-hover:gap-2">View project <Icon name="arrow-right" className="h-3.5 w-3.5"/></a></div></div>
    </Card>;
  })}</div>;
}
