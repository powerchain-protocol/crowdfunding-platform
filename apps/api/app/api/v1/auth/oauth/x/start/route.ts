...
export async function GET(request: Request) {
  try {
    ...
    const response = NextResponse.redirect(buildAuthorizationUrl("x", state, challenge));
    response.cookies.set(oauthCookieName("x"), JSON.stringify({ state, verifier, returnTo, remembered }), oauthCookieOptions());
    return response;
  } catch (error) {
