...
export default function Page(){return <AppShell appName="Audit" nav={[{label:"Admin",href:"/"},{label:"Moderation",href:"/moderation"},{label:"Audit",href:"/audit"}]}><PageHeader eyebrow="Append-only audit ledger" title="Every privileged action leaves a tamper-evident trail" description="Events are sequence-ordered and hash-linked. The log is designed to reveal alteration; production storage should add immutable/WORM retention controls."/><Section title="Recent events"><Card className="divide-y divide-neutral-200">{events.map(([seq,type,resource,hash,time])=><div key={seq} className="grid gap-3 p-5 md:grid-cols-[90px_1fr_210px_90px] md:items-center"><StatusBadge>{seq}</StatusBadge><div><div className="text-sm font-bold">{type}</div><div className="mt-1 font-mono text-[10px] text-neutral-400">{resource}</div></div><code className="font-mono text-xs text-neutral-500">{hash}</code><div className="text-xs text-neutral-400">{time}</div></div>)}</Card></Section></AppShell>}
