...
export function MarketingHome() {
  return <AppShell appName="Funding & Capital Infrastructure" accountMode="website" authHref="/auth/login" nav={[{label:"Explore projects",href:"http://localhost:3001/explore"},{label:"Investor marketplace",href:"http://localhost:3001/marketplace"},{label:"How it works",href:"/how-it-works"},{label:"About",href:"/about"},{label:"Docs",href:"http://localhost:3008"}]}>
    <section className="pc-grid-bg relative overflow-hidden rounded-[28px] border border-neutral-200 bg-white px-6 py-10 shadow-[0_24px_70px_rgba(0,0,0,.06)] sm:px-10 lg:px-14 lg:py-16">
      ...
      <div className="relative grid gap-12 lg:grid-cols-[1.06fr_.94fr] lg:items-center">
        <div>
          ...
          <div className="mt-8 grid max-w-xl gap-3 sm:grid-cols-2"><a href="http://localhost:3001/explore" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-emerald-950 px-5 text-sm font-bold text-white shadow-sm transition hover:-translate-y-px hover:bg-emerald-900 hover:shadow-md">Explore projects <Icon name="arrow-right" /></a><a href="http://localhost:3001/create" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-neutral-200 bg-white px-5 text-sm font-bold text-neutral-950 transition hover:-translate-y-px hover:border-neutral-300 hover:shadow-sm">Create campaign</a></div>
    ...
    <section className="mt-12 overflow-hidden rounded-[26px] bg-emerald-950 px-6 py-9 text-white sm:px-10 lg:flex lg:items-center lg:justify-between lg:gap-8"><div><div className="text-[11px] font-bold uppercase tracking-[.16em] text-emerald-200">Start with transparency built in</div><h2 className="mt-3 text-2xl font-bold tracking-[-.03em] sm:text-3xl">Create a campaign or founder raise with verification, milestone terms and escrow controls from day one.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/70">Founders and campaign operators get structured workflows while investors/supporters see verified terms, proof gates and transparent fund movement.</p></div><div className="mt-6 grid shrink-0 gap-2 sm:grid-cols-2 lg:mt-0 lg:grid-cols-1 xl:grid-cols-2"><a href="http://localhost:3001/create" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-white px-5 text-sm font-bold text-emerald-950 shadow-sm transition hover:-translate-y-px">Create campaign <Icon name="arrow-right"/></a><a href="http://localhost:3001/marketplace" className="inline-flex h-11 items-center justify-center rounded-xl border border-white/25 px-5 text-sm font-bold text-white transition hover:-translate-y-px hover:bg-white/10">Investor marketplace</a></div></section>
  </AppShell>;
}
