...
if (generator && !/provider\s*=\s*"prisma-client-js"/.test(generator)) {
  errors.push('Generator provider must be "prisma-client-js" for Prisma 6.19.3.');
}
if (datasource && !/provider\s*=\s*"postgresql"/.test(datasource)) {
  errors.push('Datasource provider must be "postgresql".');
}
if (datasource && !/url\s*=\s*env\("DATABASE_URL"\)/.test(datasource)) {
  errors.push('Datasource URL must read DATABASE_URL.');
}
...
for (const char of schema) {
  ...
  if (char === "}") depth -= 1;
  if (depth < 0) {
    errors.push("Unbalanced closing brace in Prisma schema.");
    break;
  }
}
...
console.log(`Prisma schema preflight OK: ${modelNames.length} models, canonical generator/datasource blocks.`);
